...
extern HRESULT PassRec_CreateInstance(__in REFIID riid, __deref_out void** ppv);
EXTERN_C GUID CLSID_PassCred;
...
public:
    ...
    IFACEMETHODIMP CreateInstance(__in IUnknown* pUnkOuter, __in REFIID riid, __deref_out void **ppv)
    {
        ...
        if (!pUnkOuter)
        {
            hr = PassRec_CreateInstance(riid, ppv);
        }
        else
        {
            *ppv = NULL;
